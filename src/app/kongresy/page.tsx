...
export default async function KongressyPage() {
  ...
  const { data: kongresy, error } = await supabase
    .from('kongresy')
  ...
  if (error) {
    console.error('Chyba při načítání kongresů:', error)
  }
        ...
        <div className="grid gap-4">
          {kongresy.map((kongres) => (
            ...
            >
              ...
              {kongres.misto && (
                <p className="text-gray-600 mt-1">{kongres.misto}</p>
              )}
            </Link>
          ))}
        </div>
